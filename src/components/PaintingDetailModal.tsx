...
export default function PaintingDetailModal({ painting, onClose, theme = 'dark' }: PaintingDetailModalProps) {
  ...
  const [wallColor, setWallColor] = useState<'charcoal' | 'linens' | 'sage' | 'burgundy'>('charcoal');
      ...
      <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-stone-950/75 backdrop-blur-xs">
        ...
        >
          ...
          <div className={`md:col-span-7 p-6 md:p-8 flex flex-col items-center justify-between border-b md:border-b-0 md:border-r transition-colors duration-300 relative ${
            theme === 'dark' ? 'bg-[#0E0D0C] border-stone-850' : 'bg-stone-100 border-stone-200/70'
          }`}>
            <div className={`absolute inset-0 pointer-events-none ring-1 ring-inset transition-opacity duration-300 ${
              theme === 'dark' ? 'ring-white/5 opacity-40 shadow-[inset_0_0_40px_rgba(0,0,0,0.4)]' : 'ring-black/5 shadow-[inset_0_0_40px_rgba(0,0,0,0.04)]'
            }`} />
              ...
              {viewMode === 'frame' ? (
                ...
                >
                  ...
                  <div className={`relative transition-all duration-300 bg-white select-none ${
                    frameStyle === 'canvas' 
                      ? 'border-2 border-stone-200 p-0 frame-shadow-canvas'
                      : frameStyle === 'oak' 
                        ? 'border-[16px] border-[#D7C4A5] bg-[#FAF8F5] p-8 frame-shadow-oak'
                        : frameStyle === 'black'
                          ? 'border-[16px] border-[#1E1C1A] bg-[#FAF8F5] p-8 frame-shadow-black'
                          : 'border-[16px] border-[#D4AF37] bg-[#FAF8F5] p-8 frame-shadow-gold ring-1 ring-yellow-605/30'
                  }`}>
                ...
                <div className={`relative w-full h-[380px] rounded-lg overflow-hidden border shadow-inner flex flex-col justify-between items-center p-6 transition-all duration-500 ${
                  ...
                  wallColor === 'linens' ? 'bg-[#ECEAE4] border-stone-300 text-stone-800 shadow-[inset_0_4px_15px_rgba(0,0,0,0.12)]' :
                  wallColor === 'sage' ? 'bg-[#6E7A6E] border-stone-800 text-stone-100 shadow-[inset_0_4px_20px_rgba(0,0,0,0.25)]' :
                  'bg-[#4C2A2F] border-stone-950 text-stone-100 shadow-[inset_0_4px_25px_rgba(0,0,0,0.5)]'
                }`}>
                  {/* Spotlight projection */}
                  <div className="absolute inset-0 pointer-events-none wall-spotlight opacity-100" />
                  ...
                  <div className="absolute -top-1 w-10 h-3 bg-stone-900 border border-white/10 rounded-b-full flex items-center justify-center shadow-md">
                    <div className="w-4 h-1.5 bg-amber-200 rounded-full animate-pulse blur-xs" />
                  </div>

                  {/* Canvas scale dimensions tag */}
                  <div className="absolute top-4 left-4 font-mono text-[9px] uppercase tracking-wider bg-black/40 text-stone-300 p-1 px-2 rounded-xs pointer-events-none select-none">
                  ...
                  <div className="flex-grow flex items-center justify-center mt-3 z-10">
                    <div className={`relative transition-all duration-300 bg-white select-none scale-[0.6] md:scale-[0.7] transform origin-center ${
                      frameStyle === 'canvas' 
                        ? 'border border-stone-350 p-0 frame-shadow-canvas'
                        : frameStyle === 'oak' 
                          ? 'border-[12px] border-[#D7C4A5] bg-[#FAF8F5] p-5 frame-shadow-oak'
                          : frameStyle === 'black'
                            ? 'border-[12px] border-[#1E1C1A] bg-[#FAF8F5] p-5 frame-shadow-black'
                            : 'border-[12px] border-[#D4AF37] bg-[#FAF8F5] p-5 frame-shadow-gold ring-1 ring-yellow-605/30'
                    }`}>
                      <img
                        src={painting.imageUrl}
                        alt={painting.title}
                  ...
                  <div className="w-3/5 h-20 border-t-2 border-x border-stone-700/50 bg-[#1E1D1A]/5 backdrop-blur-xs flex flex-col justify-end relative mt-auto z-10">
                    ...
                    <div className="text-[8px] uppercase tracking-widest font-sans font-bold text-center text-stone-500/55 select-none pb-2">
              ...
              {viewMode === 'frame' ? (
                ...
                <div className="flex flex-col items-center gap-2">
                  <span className={`text-[10px] font-sans font-bold uppercase tracking-wider flex items-center gap-1.5 ${
                    ...
                    <Palette className="w-3.5 h-3.5 text-amber-505" />
                      ...
                      return (
                        <button
                          ...
                          className={`w-6 h-6 rounded-full border-2 transition-all cursor-pointer hover:scale-110 shadow-xs ${w.bg} ${
                            isActive ? 'ring-2 ring-amber-555 ring-offset-2 ring-offset-[#0d0c0b]' : 'ring-0'
                          }`}
                          title={w.name}
                          aria-label={w.name}
                        />
          ...
          <div className={`md:col-span-5 p-6 md:p-8 flex flex-col justify-between overflow-y-auto max-h-[85vh] md:max-h-[650px] transition-colors duration-300 ${
            theme === 'dark' ? 'bg-[#131211] text-stone-200' : 'bg-white text-stone-900'
          }`}>
            <div>
              {/* Category Breadcrumb */}
              <div className="flex flex-wrap items-center gap-2 mb-3">
                <span className={`text-[9px] uppercase font-sans font-bold tracking-widest px-2 py-1 rounded-sm border ${
                  theme === 'dark' ? 'bg-stone-900 text-amber-450 border-stone-850' : 'bg-stone-105 text-amber-800 border-stone-200/50'
                }`}>
                  {painting.category}
                </span>
                <span className="text-stone-500 font-sans">•</span>
                <span className="text-[10px] font-sans text-stone-400 font-semibold uppercase tracking-wider">
              ...
              <p className={`font-serif italic text-sm mt-1.5 ${theme === 'dark' ? 'text-stone-400' : 'text-stone-605'}`}>
                {painting.medium}, {painting.year}
              </p>

              <div className={`flex items-center gap-4 mt-4 py-2.5 px-3 border rounded-md ${
                theme === 'dark' ? 'bg-stone-950/40 border-stone-850' : 'bg-stone-50 border-stone-150/60'
              }`}>
                <div className="flex items-center gap-1.5 text-xs font-sans">
                  <SlidersHorizontal className="w-3.5 h-3.5 text-stone-400" />
                  <span>{painting.dimensions}</span>
                </div>
                <div className={`w-px h-4 ${theme === 'dark' ? 'bg-stone-800' : 'bg-stone-200'}`} />
                <div className="flex items-center gap-1.5 text-xs font-sans">
            ...
            <div className={`mt-8 pt-6 border-t ${theme === 'dark' ? 'border-stone-850' : 'border-stone-150'}`}>
              <div className="flex items-baseline justify-between mb-4">
                <span className="text-xs text-stone-500 font-sans">Status</span>
                <div className="flex items-center gap-2">
                  <span className={`w-2 h-2 rounded-full ${isAvailable ? 'bg-emerald-500 animate-pulse' : 'bg-stone-455'}`} />
              ...
              {painting.price !== null ? (
                <div className="flex items-center justify-between mb-5">
                  ...
                  <span className={`font-serif text-2xl font-bold ${theme === 'dark' ? 'text-amber-400' : 'text-stone-950'}`}>
              ...
              <div className={`border p-4 rounded-lg transition-all ${
                theme === 'dark' ? 'bg-stone-950/40 border-stone-850' : 'bg-stone-50 border-stone-200/60'
              }`}>
                {isSubmitted ? (
                  <motion.div
                  ...
                  >
                    ...
                    <h5 className="font-serif font-semibold text-stone-100 text-sm">Inquiry Sent Successfully</h5>
                    <p className="font-sans text-xs text-stone-400 mt-1 max-w-[280px]">
                      Your inquiry regarding <strong>{painting.title}</strong> has been cataloged. The studio will contact you promptly.
                    </p>
                    <button
                      type="button"
                      onClick={() => setIsSubmitted(false)}
                      className="text-amber-450 hover:text-amber-350 font-sans font-semibold text-[11px] underline mt-3 cursor-pointer"
                    >
                  ...
                  <form onSubmit={handleSubmitInquiry} className="space-y-3">
                    ...
                    <button
                      ...
                      className={`w-full font-sans text-xs font-bold uppercase tracking-widest py-3 px-4 rounded-sm flex items-center justify-center gap-1.5 cursor-pointer transition-all ${
                        theme === 'dark' 
                          ? 'bg-amber-550 hover:bg-amber-450 text-stone-950' 
                          : 'bg-stone-900 hover:bg-amber-905 text-white'
                      }`}
                    >
                      {isSubmitting ? (
